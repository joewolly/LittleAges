    ...
    public async Task ProductionOpenUpgradesActualM0AndRetainsCanonicalWorldOnReopen()
    {
        await WithDatabaseAsync(async path =>
        {
            ...
            await using (var database = await WorldDatabase.OpenAsync(path, new WorldDatabaseOpenOptions(upgradeUtc)))
            {
                ...
                Assert.Equal(["20260912000000_InitialM0", "20260912010000_M1World", "20260912020000_M2Citizens", "20260912030000_M3Survival", "20260912040000_M4Settlement", "20260912050000_M5Social", "20260912060000_M6History", "20260920010000_LivingSettlement"], migrations.ToArray());
