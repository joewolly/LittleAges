...
Configuration keys are `DataRoot`, `ListenUrls`, `ActiveWorld`, `WorldSeed`, `SimulationMinutesPerSecond`, `CheckpointSimulationMinutes`, `CheckpointMinimumRealSeconds`, `CheckpointRetryCount`, `CheckpointRetryDelaySeconds`, `BrowserUpdateIntervalMilliseconds`, and `ObserverStreamIntervalMilliseconds`. Defaults are a data directory below the application base directory, `http://127.0.0.1:5274`, `default-world`, seed `0`, simulation rate `10` minutes per real second, checkpoint every `360` simulation minutes with a `30` real-second minimum, `3` retries after the initial checkpoint attempt with a `2` second delay, browser invalidation every `500` milliseconds, and connected observer frames every `100` milliseconds. The active world database is `<DataRoot>\<ActiveWorld>.db` (with platform path separators).
