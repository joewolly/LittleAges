# Little Ages M0-M6 Architecture

This document describes the implementation that exists through M6. The product design and implementation plan remain preserved separately; this is an implementation record, not a promise that later-milestone systems already exist.
...
There is no SignalR hub, PixiJS renderer, static frontend serving, or mutation endpoint. M5 social/family/aging and M6 history are implemented in the canonical simulation engine; M7 persistent-host/reconnect hardening and M8 scale/tuning remain deferred.
...
M0 foundations, M1 generation/persistence, M2 citizens/movement, M3 survival, M4 settlement/construction, M5 social/lifecycle, and M6 history are implemented. The current rules version is `m6-rng1-history1`.
