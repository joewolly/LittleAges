...
SimulationRulesVersion:   m2-rng1-citizen1
...
Generation version `1`, terrain/resource integer values, configuration canonicalization, field keys, retry derivation, interpolation arithmetic, resource formulas, viability score, ordering, and fingerprint encoding are compatibility data. The RNG algorithm itself is version `1`; changing any of these without a compatibility/version decision changes canonical worlds. M2 adds `CitizenGenerationVersion = 1` and current `SimulationRulesVersion = m2-rng1-citizen1`; the known pre-M2 `m0-rng1` value remains accepted only as an upgrade input. Citizen roster fingerprints use one length-prefixed UTF-8 field per explicit invariant value, in ordinal order: generation version, seed, ordinal, ID, given/family names, birth, location x/y, health, hunger/rest/shelter/social, needs-updated minute, all traits, all skills, action, action sequence, action timing, target, and lifetime movement state.

## M1 persistence and read boundary

M1 persistence adds `world_tiles` and `resource_nodes` tables and extends `world_meta` with `generation_version`, `generation_attempt`, `starting_x`, `starting_y`, and stored `world_fingerprint`. M2 adds `citizen_generation_version` and explicit `citizens` columns. Checkpointing writes metadata, map, resources, events, and citizens transactionally. M2 tuning constants are centralized in `CitizenSimulationRules`: founder count 20, idle duration 30..90 minutes, rest duration 120 minutes, wander radius 4, explore radius 12, and operational advancement defaults to 10 simulated minutes per real second (zero disables the driver). Loading rejects incomplete or inconsistent M2 state rather than regenerating it. Citizen event payloads are canonical JSON objects with exactly `citizenId` (positive invariant decimal string) and `actionSequence` (non-negative integer); legacy synthetic events retain `{}`. Weighted A* uses fixed N, NE, E, SE, S, SW, W, NW neighbor order, destination movement cost, and no diagonal corner cutting; queue ties are `F`, `H`, row-major tile index, then local insertion sequence.
...
The persisted `citizens` table has one row per founder and the following explicit fields: positive `id`, `founder_ordinal` (0..19), given/family names, signed `birth_minute`, nullable `death_minute`/`death_cause`, nullable parent A/parent B/partner/household/home-structure IDs, walkable `location_x`/`location_y`, `health`, four needs, six traits, six skills, `current_action`, `action_sequence`, nullable action start/completion minutes, nullable target x/y, `needs_updated_minute`, and lifetime movement steps/cost. M2 founders have all future lifecycle fields null, health exactly 10000, and skills are immutable. The table uses a unique founder-ordinal index and range/action constraints; persistence orders rows by `id`.

Founder generation is version `1`, seeded by the persisted UInt64 seed and ordinal/field keys from `RandomDomain.CitizenGeneration`; fixed repository-owned given/family catalogs and deterministic family collision resolution are compatibility data. Exactly 20 founders are allocated from the shared entity counter and placed on walkable tiles sorted by squared distance to the starting site, then row-major. A founder age is selected in 18..45 and `birth_minute = checked(current_minute - checked(age * 518400 + offset))`, where offset is a deterministic minute within the year; age is derived with checked signed arithmetic. Traits are six independent integer values in 0..10000. Skills are six non-negative integer values and never advance in M2.

Needs are pure integer projections with rates Hunger `2`, Rest `3`, Shelter `1`, Social `1` per minute, saturated to 10000 with overflow-safe arithmetic. Observer reads never mutate stored needs. M2 behavior only uses Idle, Rest, Wander, and Explore. Utility constants are Rest weight `2`, Explore base `1000`, Wander base `700`, Idle base `500`; ties use the explicit action order Rest, Explore, Wander, Idle. Idle duration is deterministic 30..90 minutes, Rest is 120 minutes, Wander radius is 4, and Explore radius is 12. Rest is the only behaviorally active need-reducing action; no health, death, resource, survival, relationship, household, or skill effects exist until later milestones.
...
Stable event names are `citizen.decision.v1`, `citizen.move-step.v1`, and `citizen.action-complete.v1`. Priorities are movement `10`, completion `15`, and decision `20`, so movement/completion precede a same-minute decision. Citizen payload JSON is canonical and lossless: exactly two properties, `{"citizenId":"<positive invariant decimal>","actionSequence":<non-negative integer>}`. Legacy synthetic events retain `{}`. Event identity, entity sort key, citizen existence, action sequence, current action, and due-minute/state coherence are validated structurally; unknown, duplicate, missing, or past-due citizen events reject. Processed-event accounting is a monotonic total count plus a bounded 32-entry diagnostic queue; this replaces unbounded retention while preserving legacy synthetic receipts.

M2 restoration requires exactly 20 rows, ordinals 0..19, unique positive IDs and names, valid walkable locations and ranges, null future fields, valid action/timing/target state, and exactly one coherent next citizen event per founder. Checkpoint writes metadata, world rows, resources, events, and citizens in one transaction. `citizen_generation_version=0`, zero citizen rows/events, and exactly pre-M2 `m0-rng1` rules are the strict M1 sentinel. A one-time M1→M2 upgrade preserves seed, minute, map fingerprint, counters, existing events, and `CreatedUtc`, allocates 20 shared entity IDs, and queues decisions at the current minute. Direct M0→M2 opens chain the existing M0→M1 upgrade first. Unknown/partial/corrupt state rejects; injected failure rolls back atomically and a retry regenerates once only.

The server keeps `SimulationHost` as the sole writer. `SimulationMinutesPerSecond` must be finite and non-negative; default is `10`, and `0` disables operational advancement. Fractional rates accumulate and floor whole simulated minutes; advancement is serialized through the host command path. Shutdown performs a final checkpoint. `GET /api/v1/citizens` publishes an immutable ID-sorted roster and `GET /api/v1/citizens/{id}` accepts only canonical positive decimal IDs (malformed 400, missing 404); ordinary status contains population only, not the full roster. The frontend is a read-only accessible polling observer with loading/error/list states, safe string IDs, no map/PixiJS/SignalR/canonical-state ownership, and no citizen mutation endpoint.
