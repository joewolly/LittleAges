...
public sealed class WorldCheckpointStore
{
    ...
    private async Task CheckpointCoreAsync(SimulationPersistenceSnapshot snapshot, DateTime checkpointUtc, CheckpointFailurePoint? failurePoint, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (snapshot.Economy is { } economy)
            {
                ...
                else economyRow.CanonicalJson = canonicalEconomyJson;
            }
            else if (economyRow is not null) throw new InvalidDataException("An agricultural checkpoint cannot be replaced by legacy state.");
    ...
    public async Task<SimulationPersistenceSnapshot> LoadAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var economyRow = await _context.EconomyStates.AsNoTracking().SingleOrDefaultAsync(cancellationToken);
            EconomyState? economy;
            try { economy = economyRow is null ? null : unifiedRules ? EconomyState.ParseUnified(economyRow.CanonicalJson) : EconomyState.Parse(economyRow.CanonicalJson); }
            catch (JsonException exception) { throw new InvalidDataException("Malformed economy state.", exception); }
    ...
    public async Task<bool> HasCheckpointAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (metadataCount == 0 &&
            (await _context.EconomyStates.AnyAsync(cancellationToken) || await _context.AgricultureStates.AnyAsync(cancellationToken) ||
    ...
    private static CitizenRow ToCitizenRow(Citizen citizen, bool unifiedRules = false) => new()
    {
    ...
    private static Citizen FromCitizenRow(CitizenRow row, WorldMap world, WorldMinute minute, int survivalVersion, bool livingRules, bool unifiedRules)
    {
        if ((row.CurrentAction < 0 || row.CurrentAction > (unifiedRules ? 16 : livingRules ? 13 : 15)) || row.Id <= 0 || row.FounderOrdinal is < 0 or > 19 || (survivalVersion == 0 && row.Health != 10000) || (row.FounderOrdinal is not null && row.BirthMinute >= 0) || row.LocationX < 0 || row.LocationY < 0 || row.LocationX >= world.Width || row.LocationY >= world.Height || !world.GetTile(row.LocationX, row.LocationY).Walkable) throw new InvalidDataException("Citizen row contains invalid canonical state.");
