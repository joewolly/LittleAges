...
public sealed class Citizen : IEquatable<Citizen>
{
    ...
    public Citizen Validate(WorldMap? world = null)
    {
        ...
        if (CurrentAction is CitizenAction.GatherFood or CitizenAction.GatherWood or CitizenAction.GatherStone or CitizenAction.HaulHarvest)
        {
        ...
        if (ActionPhase == CitizenActionPhase.ReturnToStockpile && (CurrentAction is not (CitizenAction.GatherFood or CitizenAction.GatherWood or CitizenAction.GatherStone or CitizenAction.HaulHarvest) || CarriedResourceQuantity <= 0 || ActionTarget is null)) throw new ArgumentException("A returning gatherer requires carried goods and a target.");
        if (CurrentAction is CitizenAction.HaulConstruction or CitizenAction.TradeDelivery && (ActionPhase is not (CitizenActionPhase.TravelToStockpile or CitizenActionPhase.TransportToConstruction) || TargetStructureId is null || ActionStartedMinute is null || ActionCompletesMinute is null || (ActionPhase == CitizenActionPhase.TravelToStockpile && (ActionTarget is null || CarriedResourceQuantity != 0)) || (ActionPhase == CitizenActionPhase.TransportToConstruction && (ActionTarget is null || CarriedResourceQuantity <= 0)))) throw new ArgumentException("Construction hauling phase state is invalid.");
        if (CurrentAction is CitizenAction.Build or CitizenAction.WorkFarm && (ActionPhase is not (CitizenActionPhase.TravelToTarget or CitizenActionPhase.Perform) || TargetStructureId is null || ActionStartedMinute is null || ActionCompletesMinute is null || (ActionPhase == CitizenActionPhase.TravelToTarget && ActionTarget is null) || (ActionPhase == CitizenActionPhase.Perform && ActionTarget is not null))) throw new ArgumentException("Construction building phase state is invalid.");
        if (CurrentAction == CitizenAction.HaulHarvest && (TargetStructureId is null || CarriedResourceQuantity > 0 && CarriedResourceType != ResourceType.Food)) throw new ArgumentException("Harvest hauling requires a farm and food.");
