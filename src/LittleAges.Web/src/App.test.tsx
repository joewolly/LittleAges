...
describe('citizen observer', () => {
  ...
  it('renders operational controls and sends bounded control requests', async () => {
    ...
    expect(screen.getByRole('option', { name: 'Very fast · 6× · 2:47/day' })).toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'Pause' }))
    await waitFor(() => expect(screen.getByRole('button', { name: 'Resume' })).not.toBeDisabled())
    expect(screen.getByRole('option', { name: 'Custom · 10 min/s' })).toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'Resume' }))
    fireEvent.change(screen.getByLabelText('Simulation speed'), { target: { value: '4.32' } })
    ...
    expect(fetchMock.mock.calls.some(([path, request]) => String(path).endsWith('/control/speed') && request?.body === '{"speed":4.32}')).toBe(true)
  })
