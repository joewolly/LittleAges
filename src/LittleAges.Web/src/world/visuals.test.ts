...
import { citizenPaletteIndex, containMap, detailVariant, elevationAt, movementPlanIdentity, positionAlongMovementPlan, presentationSegmentProgress, reconcileVisualMinute, scenePointAlongMovementPlan, settlementSiteScenePoint, shouldInterpolateCitizen, stableVisualHash, worldToScene } from './visuals'
...
describe('diorama visual projection', () => {
  ...
  it('maps canonical coordinates into centered scene coordinates with compressed relief', () => {
    ...
    expect(settlementSiteScenePoint(map, { x: 1, y: 0 })).toEqual({ x: 0.5, y: 1.5, z: -0.5 })
  })
