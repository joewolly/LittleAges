...
function SettlementMap({ map, structures, citizens }: { map: Map; structures: Structure[]; citizens: Citizen[] }) {
  ...
  useEffect(() => {
    ...
    const padding = 20; const scale = Math.max(2, Math.min(24, Math.floor(Math.min((canvas.width - padding * 2) / map.width, (canvas.height - padding * 2) / map.height))))
    const drawWidth = map.width * scale; const drawHeight = map.height * scale; const offsetX = Math.floor((canvas.width - drawWidth) / 2); const offsetY = Math.floor((canvas.height - drawHeight) / 2)
    ...
    for (const structure of structures) { const x = offsetX + structure.location.x * scale; const y = offsetY + structure.location.y * scale; context.fillStyle = structureColors[structure.type]; context.globalAlpha = structure.status === 'Complete' ? 1 : 0.52; context.fillRect(x + scale * 0.18, y + scale * 0.18, scale * 0.64, scale * 0.64); context.globalAlpha = 1; if (structure.status === 'UnderConstruction') { context.strokeStyle = '#302a24'; context.setLineDash([Math.max(1, scale / 5), Math.max(1, scale / 6)]); context.strokeRect(x + scale * 0.11, y + scale * 0.11, scale * 0.78, scale * 0.78); context.setLineDash([]) } }
    context.fillStyle = '#302a24'; for (const citizen of citizens.filter(entry => entry.isAlive)) context.fillRect(offsetX + citizen.location.x * scale + scale * 0.42, offsetY + citizen.location.y * scale + scale * 0.42, Math.max(2, scale * 0.18), Math.max(2, scale * 0.18))
  }, [citizens, map, structures])
  return <figure className="map-figure" aria-labelledby="map-heading"><canvas ref={canvasRef} className="settlement-map" width="760" height="420" role="img" aria-label={`Settlement map, ${map.width} by ${map.height} tiles. The white outline marks the starting site; colored squares mark structures; dark points mark living citizens.`} /><figcaption><strong id="map-heading">The settlement grounds</strong><span>Terrain is a presentation of the authoritative map. Starting site is outlined in white.</span></figcaption><ul className="map-legend" aria-label="Map legend"><li><i className="legend-start" />Starting site</li><li><i className="legend-shelter" />Shelter</li><li><i className="legend-stockpile" />Stockpile</li><li><i className="legend-workshop" />Workshop</li><li><i className="legend-citizen" />Citizen</li></ul></figure>
}
...
export function App() {
  ...
  return <main className="page-shell"><div className="topline"><span className="mark" aria-hidden="true">✦</span><span>Observer station</span><span className="rule" /></div><header className="hero"><p className="eyebrow">A world in quiet motion</p><h1>Little <em>Ages</em></h1><p className="intro">A small window onto a persistent world.<br className="desktop-break" /> The server keeps time; this page simply looks in.</p></header><section className="status-strip" aria-label="Connection status" role="status" aria-live="polite"><span className={`status-dot ${connected ? 'is-good' : ''}`} aria-hidden="true" /><span>{error ? 'Connection unavailable' : health === null ? 'Checking the server…' : connected ? 'Server connected' : `Server: ${health.label}`}</span><span className="live-status">Live updates: {liveLabel}</span><span className="status-detail">{liveConnectionState === 'connected' ? 'Observer connected' : `Degraded · REST fallback every ${REST_FALLBACK_INTERVAL_MS / 1000}s`}</span></section>{error && <div className="notice" role="alert">{error}. Make sure the Little Ages server is running.</div>}{mapError && <div className="notice" role="alert">Map unavailable: {mapError}.</div>}{status.error && <div className="notice" role="alert">Server reports: {status.error}</div>}
    ...
    <section className="control-surface" aria-labelledby="control-heading"><div className="section-heading"><span className="section-kicker">Operational controls</span><h2 id="control-heading">Set the pace</h2><p className="section-description">Pause or adjust operational pacing. These controls do not alter the canonical world state.</p></div><div className="control-actions"><button type="button" className="observer-button" onClick={() => { void updateControl(pauseSimulation) }} disabled={controlBusy || status.paused}>Pause</button><button type="button" className="observer-button" onClick={() => { void updateControl(resumeSimulation) }} disabled={controlBusy || !status.paused}>Resume</button><label>Speed<select aria-label="Simulation speed" value={status.operationalSpeed ?? 10} onChange={event => { void updateControl(() => changeSimulationSpeed(Number(event.target.value))) }} disabled={controlBusy}>{SAFE_OPERATIONAL_SPEEDS.map(speed => <option key={speed} value={speed}>{speed} minutes / second</option>)}</select></label></div>{controlError && <p className="notice" role="alert">{controlError}</p>}</section>
