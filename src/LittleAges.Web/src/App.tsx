import { fetchLivingWorld, type LivingWorld } from './living'
import { LivingRecords, LivingCitizenRecord } from './LivingRecords'
...
import {
  ...
  fetchRelationships,
  fetchSettlement,
  fetchSettlementSites,
  fetchStatistics,
  fetchStatus,
  fetchStructures,
  changeSimulationSpeed,
  DEFAULT_OPERATIONAL_SPEED,
  OPERATIONAL_SPEED_OPTIONS,
  pauseSimulation,
  resumeSimulation,
  type Citizen,
...
const OBSERVER_TABS = ['Overview', 'Citizens', 'Buildings', 'Living', 'History', 'Statistics'] as const
...
export function App() {
  ...
  useEffect(() => {
    ...
    const loadAuthoritative = async (includeMap: boolean): Promise<boolean> => {
      ...
      const corePromise = Promise.allSettled([fetchHealth(), fetchStatus(), fetchCitizens(), fetchStructures(), fetchLivingWorld()])
      ...
      const [healthResult, statusResult, citizensResult, structuresResult, livingResult] = coreResults
      try {
        ...
        if (streamedRevision === startingStreamRevision) {
          if (livingResult.status === 'fulfilled') { setLiving(livingResult.value); setLivingError(null) } else setLivingError(errorText(livingResult.reason, 'Living settlement unavailable.'))
    ...
    const requestDetailsRefresh = (notify = true, includeHealth = true): Promise<void> => {
      ...
      const run = Promise.allSettled([includeHealth ? fetchHealth() : Promise.resolve(null), fetchSettlement(), fetchHouseholds(), fetchSettlementSites()]).then(results => {
        if (!active) return
        const [healthResult, settlementResult, householdsResult, settlementSitesResult] = results
        ...
        if (settlementSitesResult.status === 'fulfilled') setSettlementSites(settlementSitesResult.value)
      ...
      try {
        if (connection === null) {
          ...
          connection.onWorldFrame(frame => {
            ...
            if (frame.living !== undefined) { setLiving(frame.living); setLivingError(null) }
          ...
          connection.onReconnected(() => { if (active) { streamedRevision = -1; requestRevision += 1; signalRConnected = true; reconnecting = false; setLiveConnectionState('connected'); if (mapLoaded) clearFallback(); else scheduleFallback(); void requestRefresh(); void requestDetailsRefresh() } })
  ...
  const selectedOperationalSpeed = status.operationalSpeed ?? DEFAULT_OPERATIONAL_SPEED
  const hasPresetOperationalSpeed = OPERATIONAL_SPEED_OPTIONS.some(option => option.value === selectedOperationalSpeed)
  ...
  return <main className={`page-shell ${recordsOpen ? 'has-records-open' : ''}`}><div className="topline"><span className="mark" aria-hidden="true">✦</span><span>Observer station</span><span className="rule" /></div><header className="hero"><p className="eyebrow">A world in quiet motion</p><h1>Little <em>Ages</em></h1><p className="intro">A living miniature world.<br className="desktop-break" /> The server keeps time; this page simply looks in.</p></header><section className="status-strip" aria-label="Connection status" role="status" aria-live="polite"><span className={`status-dot ${connected ? 'is-good' : ''}`} aria-hidden="true" /><span>{error ? 'Connection unavailable' : health === null ? 'Checking the server…' : connected ? 'Server connected' : `Server: ${health.label}`}</span><span className="live-status">Live updates: {liveLabel}</span><span className="status-detail">{liveConnectionState === 'connected' ? 'Observer connected' : 'Degraded · responsive REST fallback active'}</span></section>{error && <div className="notice" role="alert">{error}. Make sure the Little Ages server is running.</div>}{mapError && <div className="notice" role="alert">Map unavailable: {mapError}.</div>}{status.error && <div className="notice" role="alert">Server reports: {status.error}</div>}
    {map === null && !error && <div className="world-loading" role="status">Preparing the living diorama…</div>}{map !== null && <SceneLoadBoundary fallback={<section className="world-viewport"><p role="alert">The 3D view could not load. Showing the 2D map; reload to retry.</p><LegacyMap living={living} map={map} structures={structures ?? []} citizens={citizens ?? []} settlementSites={settlementSites ?? []} /></section>}><Suspense fallback={<div className="world-loading" role="status">Preparing the living diorama…</div>}><WorldViewport living={living} map={map} structures={structures ?? []} citizens={citizens ?? []} settlement={settlement} settlementSites={settlementSites ?? []} worldSeed={status.worldSeed} operationalSpeed={status.operationalSpeed} worldMinute={status.worldMinute ?? 0} paused={status.paused} controlsEnabled={!recordsOpen} selectedCitizenId={selectedCitizenId} onSelectCitizen={selectCitizen} onOpenSelected={openCitizenRecord} /></Suspense></SceneLoadBoundary>}
    <section className="world-hud" aria-label="World controls"><h2 className="visually-hidden">Set the pace</h2><div><span>World time</span><strong>{status.worldMinute === null ? '—' : `Current: ${formatCalendarDate(status.worldMinute)}`}</strong></div><div><span>Population</span><strong>{formatValue(status.livingPopulation ?? settlement?.livingPopulation)}</strong></div><div className="world-hud-controls"><button type="button" onClick={() => { void updateControl(status.paused ? resumeSimulation : pauseSimulation) }} disabled={controlBusy}>{status.paused ? 'Resume' : 'Pause'}</button><label>Speed<select aria-label="Simulation speed" value={selectedOperationalSpeed} onChange={event => { void updateControl(() => changeSimulationSpeed(Number(event.target.value))) }} disabled={controlBusy}>{!hasPresetOperationalSpeed && <option value={selectedOperationalSpeed}>{selectedOperationalSpeed === 0 ? 'Paused · choose pace' : `Custom · ${selectedOperationalSpeed} min/s`}</option>}{OPERATIONAL_SPEED_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label></div></section>{controlError && <div className="notice control-notice" role="alert">{controlError}</div>}
    ...
    {recordsTab === 'Citizens' && <>
    <GrowthSurface />
    <EconomySurface citizens={citizens ?? []} />
