...
public sealed class SimulationEngine
{
    ...
    public string ComputeSurvivalFingerprint()
    {
        ...
        Add(hash, $"seed={Seed.Value}"); Add(hash, $"minute={CurrentMinute.Value}"); Add(hash, $"world-schema={WorldSchemaVersion}"); Add(hash, $"world={World.Fingerprint}"); Add(hash, $"world-configuration={WorldConfiguration}"); Add(hash, $"rules={SimulationRulesVersion}"); Add(hash, $"citizen-generation={citizenGeneration}"); Add(hash, $"survival={survival}"); Add(hash, $"settlement-food={Settlement.FoodStored}"); Add(hash, $"settlement-wood={Settlement.WoodStored}"); Add(hash, $"settlement-stone={Settlement.StoneStored}"); Add(hash, $"counter-next-entity={counters.NextEntityId}"); Add(hash, $"counter-next-historical-event={counters.NextHistoricalEventId}"); Add(hash, $"counter-next-scheduled={counters.NextScheduledEventSequence}");
        foreach (var state in ResourceStates) Add(hash, $"resource={state.ResourceNodeId.Value}:{state.CurrentQuantity}");
        foreach (var citizen in _citizens.Values.OrderBy(x => x.Id.Value))
        {
            Add(hash, $"citizen-id={citizen.Id.Value}"); Add(hash, $"citizen-ordinal={citizen.FounderOrdinal}"); Add(hash, $"citizen-given={citizen.GivenName}"); Add(hash, $"citizen-family={citizen.FamilyName}"); Add(hash, $"citizen-birth={citizen.BirthMinute}"); Add(hash, $"citizen-parent-a={citizen.ParentAId?.Value.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? "null"}"); Add(hash, $"citizen-parent-b={citizen.ParentBId?.Value.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? "null"}"); Add(hash, $"citizen-partner={citizen.PartnerId?.Value.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? "null"}"); Add(hash, $"citizen-household={citizen.HouseholdId?.Value.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? "null"}"); Add(hash, $"citizen-home={citizen.HomeStructureId?.Value.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? "null"}"); Add(hash, $"citizen-location={citizen.Location.X},{citizen.Location.Y}"); Add(hash, $"citizen-health={citizen.Health}"); Add(hash, $"citizen-death-minute={citizen.DeathMinute?.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? "null"}"); Add(hash, $"citizen-death-cause={citizen.DeathCause ?? "null"}"); Add(hash, $"citizen-needs={citizen.Needs.Hunger},{citizen.Needs.Rest},{citizen.Needs.Shelter},{citizen.Needs.Social}"); Add(hash, $"citizen-needs-updated={citizen.NeedsUpdatedMinute}"); Add(hash, $"citizen-health-updated={citizen.HealthUpdatedMinute}"); Add(hash, $"citizen-traits={citizen.Traits.Industriousness},{citizen.Traits.Sociability},{citizen.Traits.Curiosity},{citizen.Traits.Cooperativeness},{citizen.Traits.RiskTolerance},{citizen.Traits.Resilience}"); Add(hash, $"citizen-skills={citizen.Skills.Foraging},{citizen.Skills.Woodcutting},{citizen.Skills.Stoneworking},{citizen.Skills.Construction},{citizen.Skills.Hauling},{citizen.Skills.Domestic}"); Add(hash, $"citizen-action={(int)citizen.CurrentAction}"); Add(hash, $"citizen-phase={(int)citizen.ActionPhase}"); Add(hash, $"citizen-sequence={citizen.ActionSequence}"); Add(hash, $"citizen-action-started={citizen.ActionStartedMinute?.Value.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? "null"}"); Add(hash, $"citizen-action-completes={citizen.ActionCompletesMinute?.Value.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? "null"}"); Add(hash, $"citizen-action-target={(citizen.ActionTarget is { } target ? $"{target.X},{target.Y}" : "null")}"); Add(hash, $"citizen-resource-target={citizen.TargetResourceNodeId?.Value.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? "null"}"); Add(hash, $"citizen-carried-type={(citizen.CarriedResourceType is { } carriedType ? ((int)carriedType).ToString(System.Globalization.CultureInfo.InvariantCulture) : "null")}"); Add(hash, $"citizen-carried-quantity={citizen.CarriedResourceQuantity}"); Add(hash, $"citizen-movement={citizen.LifetimeMovementSteps},{citizen.LifetimeMovementCost}");
        }
        foreach (var item in _scheduledEvents) Add(hash, $"event-id={item.Id.Value};due={item.Order.DueWorldMinute.Value};priority={item.Order.Priority};entity={item.Order.EntitySortKey};sequence={item.Order.Sequence};name={item.Name};payload={item.PayloadJson}");
