        ...
        IReadOnlyList<Household> households, WorldMap world, HistoryState state)
    {
        ...
        switch (historicalEvent.EventType)
        {
            ...
            case HistoricalEventType.CitizenBorn:
                ...
                {
                    if (!long.TryParse(bornHousehold.GetString(), System.Globalization.NumberStyles.None, System.Globalization.CultureInfo.InvariantCulture, out var householdId) || !households.Any(x => x.Id.Value == householdId) || born.HouseholdId?.Value != householdId) throw new ArgumentException("CitizenBorn household reference is not canonical.");
                }
                else if (historicalEvent.Origin == HistoricalEventOrigin.Live || born.HouseholdId is not null) throw new ArgumentException("CitizenBorn is missing its canonical household reference.");
                break;
...
public sealed partial class SimulationEngine
{
    ...
    public bool ProcessNextEvent()
    {
        ...
        var livingBefore = LivingPopulation;
        ...
        if (HistorySystemsEnabled(SimulationRulesVersion)) RecordHistoryTransitions(foodBefore, livingBefore);
        return true;
    }
