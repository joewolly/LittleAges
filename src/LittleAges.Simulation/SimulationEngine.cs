...
public sealed record SimulationPersistenceSnapshot
{
    ...
    public SimulationPersistenceSnapshot(WorldSeed seed, WorldMinute worldMinute, string worldSchemaVersion, string simulationRulesVersion, string applicationVersion, string worldConfiguration, DeterministicCountersSnapshot counters, IReadOnlyList<ScheduledEventSnapshot> scheduledEvents, WorldMap? world, IReadOnlyList<Citizen>? citizens, int citizenGenerationVersion = 0, IReadOnlyList<ResourceState>? resourceStates = null, SettlementState? settlement = null, int survivalVersion = 0, int settlementVersion = 0, IReadOnlyList<Structure>? structures = null, IReadOnlyList<StructureContribution>? structureContributions = null, int socialVersion = 0, IReadOnlyList<RelationshipState>? relationships = null, IReadOnlyList<Household>? households = null, int historyVersion = 0, HistoryState? historyState = null, IReadOnlyList<HistoricalEvent>? historicalEvents = null, IReadOnlyList<HistoricalEventCitizenLink>? historicalEventCitizens = null, IReadOnlyList<HistoricalEventStructureLink>? historicalEventStructures = null, IReadOnlyList<StatisticsSample>? statisticsSamples = null, IReadOnlyList<CitizenMemory>? memories = null, AgricultureState? agriculture = null, EconomyState? economy = null, string? livingStateJson = null)
    {
        ...
        Economy?.Validate(Citizens, Households, Structures, Settlement!, World!, worldMinute.Value, checked(Settlement!.BaseStorageCapacity + Structures.Count(s => s.Type == StructureType.Stockpile && s.Status == StructureStatus.Complete) * CitizenSimulationRules.StockpileStorageBonus + Structures.Count(s => s.Type == StructureType.Granary && s.Status == StructureStatus.Complete) * AgricultureRules.GranaryFoodCapacity), Structures.Count(s => s.Type == StructureType.Granary && s.Status == StructureStatus.Complete) * AgricultureRules.GranaryFoodCapacity);
...
public sealed partial class SimulationEngine
{
    ...
    public const string CurrentSimulationRulesVersion = SpacedSimulationRulesVersion;
    ...
    public static int FoodShortageRecoveryMultiplier(string rulesVersion) => rulesVersion switch
    {
        ...
        M8SimulationRulesVersion or GrowthSimulationRulesVersion or AgricultureSimulationRulesVersion or BarterSimulationRulesVersion or SpacedSimulationRulesVersion or LivingSimulationRulesVersion or Living2SimulationRulesVersion => 20,
    ...
    public string ComputeSurvivalFingerprint()
    {
        ...
        var citizenGeneration = SimulationRulesVersion is M5SimulationRulesVersion or M6SimulationRulesVersion or M8SimulationRulesVersion or GrowthSimulationRulesVersion or AgricultureSimulationRulesVersion or BarterSimulationRulesVersion or SpacedSimulationRulesVersion or LivingSimulationRulesVersion or Living2SimulationRulesVersion or M4SimulationRulesVersion or M3SimulationRulesVersion or M2SimulationRulesVersion ? CitizenGenerationVersion : 0;
        var survival = SimulationRulesVersion is M5SimulationRulesVersion or M6SimulationRulesVersion or M8SimulationRulesVersion or GrowthSimulationRulesVersion or AgricultureSimulationRulesVersion or BarterSimulationRulesVersion or SpacedSimulationRulesVersion or LivingSimulationRulesVersion or Living2SimulationRulesVersion or M4SimulationRulesVersion or M3SimulationRulesVersion ? SurvivalVersion : 0;
    ...
    private void Decide(Citizen citizen)
    {
        ...
        if (LivingEnabled && TryStartLivingWork(citizen, evaluations.Max(x => x.FinalScore))) return;
    ...
    private void Eat(Citizen citizen)
    {
        ...
        var consumed = EconomyEnabled ? ConsumeEconomicMeal(citizen) : Math.Min(portion, Settlement.FoodStored);
    ...
    private TileCoordinate? SelectConstructionSite()
    {
        ...
        if (SimulationRulesVersion != SpacedSimulationRulesVersion) return candidates.Select(x => (TileCoordinate?)x.Coordinate).FirstOrDefault();
